...
export class MultiAgentOrchestratorController {
  private orchestratorService: MultiAgentOrchestratorService;

  constructor(io: Server) {
    const agentService = new AgentService(io);
    const webhookService = new WebhookService(io);
    const pipedreamService = new PipedreamService(prisma);
    const collaborationService = new CollaborationService(prisma, io, agentService, webhookService);
    
    this.orchestratorService = new MultiAgentOrchestratorService(
      prisma,
      io,
      agentService,
  ...
  createSpecializedAgent = asyncHandler(async (req: Request, res: Response) => {
  ...
  createAgentFlowOrchestration = asyncHandler(async (req: Request, res: Response) => {
    const userId = req.user?.id;
    if (!userId) {
      return res.status(401).json({ success: false, message: 'Unauthorized' });
    }
  ...
  getOrchestrationStatus = asyncHandler(async (req: Request, res: Response) => {
    const userId = req.user?.id;
    if (!userId) {
      return res.status(401).json({ success: false, message: 'Unauthorized' });
    }
  ...
  createAgentsFromTemplate = asyncHandler(async (req: Request, res: Response) => {
    const userId = req.user?.id;
    if (!userId) {
      return res.status(401).json({ success: false, message: 'Unauthorized' });
    }
  ...
  getFrameworkTemplates = asyncHandler(async (req: Request, res: Response) => {
    const templates = {
      HyperAgent: {
        name: 'HyperAgent',
  ...
  getAgentConfigTemplate = asyncHandler(async (req: Request, res: Response) => {
