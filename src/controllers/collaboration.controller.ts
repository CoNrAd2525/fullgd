...
export class CollaborationController {
  ...
  constructor(prisma: PrismaClient, io: Server, agentService: AgentService, webhookService: WebhookService) {
    this.collaborationService = new CollaborationService(prisma, io, agentService, webhookService);
  }
  ...
  requestApproval = async (req: AuthenticatedRequest, res: Response) => {
    try {
      const { sessionId } = req.params;
      ...
      const approval = await this.collaborationService.requestApproval(
        sessionId,
        agentId,
        title,
        description,
  ...
  handleApprovalResponse = async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const approval = await this.collaborationService.handleApprovalResponse(
        approvalId,
        userId,
        approved,
        feedback
  ...
  // Get session tasks
  getSessionTasks = async (req: AuthenticatedRequest, res: Response) => {
    try {
      const { sessionId } = req.params;
