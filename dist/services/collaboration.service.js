...
class CollaborationService {
    constructor(prisma, io, agentService, webhookService) {
        ...
        this.pipedreamService = new pipedream_service_1.PipedreamService(prisma);
    }
    ...
    async createCollaborationSession(data) {
        try {
            ...
            try {
                await this.pipedreamService.sendCollaborationEvent('session_created', session.id, {
    ...
    async sendMessage(data) {
        try {
            ...
            // Send event to Pipedream
            try {
                await this.pipedreamService.sendCollaborationEvent('message_sent', data.sessionId, {
                    messageId: message.id,
    ...
    async assignTask(data) {
        try {
            ...
            // Send event to Pipedream
            try {
                await this.pipedreamService.sendCollaborationEvent('task_assigned', data.sessionId, {
                    taskId: task.id,
                    title: data.title,
                    description: data.description,
                    priority: data.priority,
    ...
    async requestApproval(sessionId, agentId, title, description, data) {
        try {
            ...
            // Send event to Pipedream
            try {
                await this.pipedreamService.sendCollaborationEvent('approval_requested', sessionId, {
                    approvalId: approval.id,
                    title,
                    description,
    ...
    async handleApprovalResponse(approvalId, userId, approved, feedback) {
        try {
    ...
    async handleApprovalRequest(message) {
        ...
        if (message.metadata?.approvalData) {
            await this.requestApproval(message.sessionId, message.fromAgentId, message.metadata.approvalData.title, message.metadata.approvalData.description, message.metadata.approvalData.data);
        }
    }
