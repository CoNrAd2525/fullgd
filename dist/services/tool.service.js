...
class ToolService {
    // Create a new tool
    async createTool(userId, toolData) {
        try {
            // Validate tool data
            if (!toolData.name || !toolData.type) {
                throw new errorHandler_1.ApiError(400, 'Tool name and type are required');
            }
            // Create tool
            ...
            if (error instanceof errorHandler_1.ApiError)
                throw error;
            throw new errorHandler_1.ApiError(500, 'Failed to create tool');
        }
    }
    // Get all tools for a user
    async getTools(userId, category) {
        try {
            ...
            throw new errorHandler_1.ApiError(500, 'Failed to get tools');
        }
    }
    // Get tool by ID
    async getToolById(id, userId) {
        try {
            ...
            if (!tool) {
                throw new errorHandler_1.ApiError(404, 'Tool not found');
            }
            return tool;
        }
        catch (error) {
            ...
            throw new errorHandler_1.ApiError(500, 'Failed to get tool');
        }
    }
    // Update tool
    async updateTool(id, userId, toolData) {
        try {
            ...
            if (!existingTool) {
                throw new errorHandler_1.ApiError(404, 'Tool not found or you do not have permission to update it');
            }
            // Update tool
            ...
            throw new errorHandler_1.ApiError(500, 'Failed to update tool');
        }
    }
    // Delete tool
    async deleteTool(id, userId) {
        try {
            // Check if tool exists and belongs to user
            const existingTool = await prisma.tool.findFirst({
                where: {
            ...
            if (!existingTool) {
                throw new errorHandler_1.ApiError(404, 'Tool not found or you do not have permission to delete it');
            }
            ...
            throw new errorHandler_1.ApiError(500, 'Failed to delete tool');
        }
    }
    ...
    async testConnection(id, userId) {
        try {
            ...
            throw new errorHandler_1.ApiError(500, 'Failed to test tool connection');
        }
    }
    // Test API connection
    async testApiConnection(tool) {
        try {
    ...
    async testDatabaseConnection(tool) {
    ...
    async getToolSchema(id, userId) {
        try {
            ...
            throw new errorHandler_1.ApiError(500, 'Failed to get tool schema');
        }
    }
    // Execute tool
    async executeTool(id, userId, params) {
        try {
            // Get tool
            const tool = await this.getToolById(id, userId);
            ...
            switch (tool.type) {
                ...
                default:
                    throw new errorHandler_1.ApiError(400, `Unsupported tool type: ${tool.type}`);
            }
        }
        catch (error) {
            if (error instanceof errorHandler_1.ApiError)
            ...
            throw new errorHandler_1.ApiError(500, 'Failed to execute tool');
        }
    }
    // Execute API tool
    async executeApiTool(tool, params) {
        try {
            const config = tool.config;
            if (!config.baseUrl) {
    ...
    async executeDatabaseTool(tool, params) {
        // This is a simplified implementation
        // In a real system, you would use the appropriate database client
        try {
            const config = tool.config;
    ...
    async connectToolToAgent(toolId, agentId, userId) {
        try {
            // Check if tool exists and belongs to user
            const tool = await prisma.tool.findFirst({
                where: {
            ...
            if (!tool) {
                throw new errorHandler_1.ApiError(404, 'Tool not found or you do not have access to it');
            }
            ...
            throw new errorHandler_1.ApiError(500, 'Failed to connect tool to agent');
        }
    }
    // Disconnect tool from agent
    async disconnectToolFromAgent(toolId, agentId, userId) {
        try {
            // Check if tool exists and belongs to user
            const tool = await prisma.tool.findFirst({
                where: {
            ...
            throw new errorHandler_1.ApiError(500, 'Failed to disconnect tool from agent');
        }
    }
    // Get tool categories
    async getToolCategories() {
        try {
            ...
            throw new errorHandler_1.ApiError(500, 'Failed to get tool categories');
        }
    }
    // Import tool from marketplace
    async importToolFromMarketplace(userId, marketplaceToolId) {
        try {
            // This is a simplified implementation
            ...
            throw new errorHandler_1.ApiError(500, 'Failed to import tool from marketplace');
        }
    }
}
