...
class AgentService {
    ...
    async createAgent(userId, agentData) {
        try {
            ...
            throw new errorHandler_1.ApiError(500, 'Failed to create agent');
        }
    }
    ...
    async getAgents(userId) {
        try {
            ...
            throw new errorHandler_1.ApiError(500, 'Failed to get agents');
        }
    }
    // Get agent by ID
    async getAgentById(id, userId) {
        try {
            ...
            if (!agent) {
                throw new errorHandler_1.ApiError(404, 'Agent not found');
            }
            return agent;
        }
        catch (error) {
            ...
            throw new errorHandler_1.ApiError(500, 'Failed to get agent');
        }
    }
    // Update agent
    async updateAgent(id, userId, agentData) {
        try {
            ...
            if (!existingAgent) {
                throw new errorHandler_1.ApiError(404, 'Agent not found or you do not have permission to update it');
            }
            ...
            throw new errorHandler_1.ApiError(500, 'Failed to update agent');
        }
    }
    // Delete agent
    async deleteAgent(id, userId) {
        try {
            ...
            if (!existingAgent) {
                throw new errorHandler_1.ApiError(404, 'Agent not found or you do not have permission to delete it');
            }
            ...
            throw new errorHandler_1.ApiError(500, 'Failed to delete agent');
        }
    }
    // Run agent
    async runAgent(id, userId, input) {
        try {
            ...
            throw new errorHandler_1.ApiError(500, 'Failed to run agent');
        }
    }
    // Execute agent (internal method)
    async executeAgent(agent, execution, input) {
        try {
            ...
            (0, socket_1.emitAgentComplete)(this.io, roomId, agent.id, result);
            ...
            // Emit error event
            const roomId = `agent:${agent.id}:execution:${execution.id}`;
            (0, socket_1.emitAgentError)(this.io, roomId, agent.id, error instanceof Error ? error.message : 'Unknown error');
    ...
    // Execute LLM agent
    async executeLLMAgent(agent, execution, input, roomId) {
        ...
        const tools = agentTools.map(agentTool => {
            const toolConfig = agentTool.tool.config;
            return {
                type: 'function',
                function: {
        ...
        for await (const chunk of stream) {
            ...
            if (content) {
                ...
                (0, socket_1.emitAgentStream)(this.io, roomId, agent.id, { type: 'text', content });
            }
            if (toolCall) {
                if (toolCall.index === 0) {
                    toolCalls.push({
        ...
        if (toolCalls.length > 0) {
            ...
            fullResponse += '\n' + finalResponse;
            // Emit final response
            (0, socket_1.emitAgentStream)(this.io, roomId, agent.id, { type: 'text', content: finalResponse });
        }
        return { response: fullResponse, toolCalls };
    }
    // Execute function agent
    async executeFunctionAgent(agent, execution, input, roomId) {
        ...
        // Emit start message
        (0, socket_1.emitAgentStream)(this.io, roomId, agent.id, { type: 'text', content: 'Executing function agent...' });
        ...
        // Emit result
        (0, socket_1.emitAgentStream)(this.io, roomId, agent.id, { type: 'text', content: JSON.stringify(result) });
        return result;
    }
    // Execute a tool
    async executeTool(agentTool, args) {
        // This is a simplified implementation
    ...
    async getContextFromKnowledgeBases(agentKnowledgeBases, query) {
        try {
            // Initialize Pinecone
    ...
    async getAgentExecutionStatus(executionId, userId) {
        try {
    ...
    async stopAgentExecution(executionId, userId) {
        try {
            const execution = await prisma.agentExecution.findUnique({
                where: { id: executionId },
                include: { agent: true }
            ...
            (0, socket_1.emitAgentError)(this.io, roomId, execution.agent.id, 'Execution stopped by user');
    ...
    // Clone agent
    async cloneAgent(id, userId, newName) {
        try {
            ...
            throw new errorHandler_1.ApiError(500, 'Failed to clone agent');
        }
    }
    // Export agent configuration
    async exportAgent(id, userId) {
        try {
            // Get the agent
            const agent = await this.getAgentById(id, userId);
            // Get agent tools
            ...
            throw new errorHandler_1.ApiError(500, 'Failed to export agent');
        }
    }
    // Import agent configuration
    async importAgent(userId, importData) {
        try {
            ...
            throw new errorHandler_1.ApiError(500, 'Failed to import agent');
        }
    }
}
