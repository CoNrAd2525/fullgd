...
class MemoryService {
    ...
    async createMemory(agentId, userId, memoryData) {
        try {
            ...
            if (!agent) {
                throw new errorHandler_1.ApiError(404, 'Agent not found or you do not have permission to access it');
            }
            ...
            throw new errorHandler_1.ApiError(500, 'Failed to create memory');
        }
    }
    // Get all memories for an agent
    async getMemories(agentId, userId, limit = 50, offset = 0) {
        try {
            // Check if agent exists and belongs to user
            const agent = await prisma.agent.findFirst({
                where: {
            ...
            throw new errorHandler_1.ApiError(500, 'Failed to get memories');
        }
    }
    // Get memory by ID
    async getMemoryById(id, agentId, userId) {
        try {
            // Check if agent exists and belongs to user
            const agent = await prisma.agent.findFirst({
                where: {
            ...
            if (!memory) {
                throw new errorHandler_1.ApiError(404, 'Memory not found');
            }
            return memory;
        }
        catch (error) {
            ...
            throw new errorHandler_1.ApiError(500, 'Failed to get memory');
        }
    }
    // Update memory
    async updateMemory(id, agentId, userId, memoryData) {
        try {
            // Check if agent exists and belongs to user
            const agent = await prisma.agent.findFirst({
                where: {
            ...
            throw new errorHandler_1.ApiError(500, 'Failed to update memory');
        }
    }
    // Delete memory
    async deleteMemory(id, agentId, userId) {
        try {
            // Check if agent exists and belongs to user
            const agent = await prisma.agent.findFirst({
                where: {
            ...
            throw new errorHandler_1.ApiError(500, 'Failed to delete memory');
        }
    }
    ...
    async clearMemories(agentId, userId) {
        try {
            ...
            throw new errorHandler_1.ApiError(500, 'Failed to clear memories');
        }
    }
    // Search memories
    async searchMemories(agentId, userId, query, limit = 5) {
        try {
            // Check if agent exists and belongs to user
            const agent = await prisma.agent.findFirst({
                where: {
            ...
            throw new errorHandler_1.ApiError(500, 'Failed to search memories');
        }
    }
    // Import memories
    async importMemories(agentId, userId, memories) {
        try {
            // Check if agent exists and belongs to user
            const agent = await prisma.agent.findFirst({
                where: {
            ...
            throw new errorHandler_1.ApiError(500, 'Failed to import memories');
        }
    }
    // Export memories
    async exportMemories(agentId, userId) {
        try {
            // Check if agent exists and belongs to user
            const agent = await prisma.agent.findFirst({
                where: {
            ...
            throw new errorHandler_1.ApiError(500, 'Failed to export memories');
        }
    }
    ...
    // Store vector in Pinecone
    async storeVectorInPinecone(memoryId, vector, agentId) {
        try {
            const pineconeClient = await initPinecone();
            if (!pineconeClient) {
                throw new Error('Failed to initialize Pinecone client');
