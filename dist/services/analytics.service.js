...
class AnalyticsService {
    ...
    async getPlatformOverview() {
        try {
            ...
            throw new errorHandler_1.ApiError(500, 'Failed to get platform overview');
        }
    }
    // Get user activity metrics
    async getUserActivityMetrics(timeframe = 'week') {
        try {
            ...
            throw new errorHandler_1.ApiError(500, 'Failed to get user activity metrics');
        }
    }
    // Get agent usage metrics
    async getAgentUsageMetrics(timeframe = 'week') {
        try {
            let startDate = new Date();
            // Set start date based on timeframe
            switch (timeframe) {
            ...
            throw new errorHandler_1.ApiError(500, 'Failed to get agent usage metrics');
        }
    }
    // Get workflow performance metrics
    async getWorkflowPerformanceMetrics(timeframe = 'week') {
        try {
            let startDate = new Date();
            // Set start date based on timeframe
            switch (timeframe) {
            ...
            throw new errorHandler_1.ApiError(500, 'Failed to get workflow performance metrics');
        }
    }
    ...
    async getResourceUsageMetrics() {
        try {
            ...
            throw new errorHandler_1.ApiError(500, 'Failed to get resource usage metrics');
        }
    }
    // Get error metrics
    async getErrorMetrics(timeframe = 'week') {
        try {
            let startDate = new Date();
            // Set start date based on timeframe
            switch (timeframe) {
            ...
            throw new errorHandler_1.ApiError(500, 'Failed to get error metrics');
        }
    }
    // Get agent analytics
    async getAgentAnalytics(agentId, timeframe = 'week') {
        try {
            ...
            throw new errorHandler_1.ApiError(500, 'Failed to get agent analytics');
        }
    }
    // Get workflow analytics
    async getWorkflowAnalytics(workflowId, timeframe = 'week') {
        try {
            ...
            throw new errorHandler_1.ApiError(500, 'Failed to get workflow analytics');
        }
    }
    // Get user analytics
    async getUserAnalytics(userId, timeframe = 'week') {
        try {
            ...
            throw new errorHandler_1.ApiError(500, 'Failed to get user analytics');
        }
    }
    // Export analytics data
    async exportAnalyticsData(type, timeframe = 'month', format = 'json') {
        try {
            let data;
            ...
            switch (type) {
                ...
                case 'errors':
                    ...
                    break;
                default:
                    throw new errorHandler_1.ApiError(400, `Invalid analytics type: ${type}`);
            }
            // Format data
            ...
            throw new errorHandler_1.ApiError(500, 'Failed to export analytics data');
        }
    }
    // Helper methods
    ...
    groupActivityByCategory(activity) {
        const groupedActivity = {};
        activity.forEach(item => {
    ...
    groupWorkflowExecutionsByDay(executions) {
        const groupedExecutions = {};
        executions.forEach(execution => {
            const date = new Date(execution.createdAt).toISOString().split('T')[0];
            if (!groupedExecutions[date]) {
    ...
    groupExecutionsByAgent(executions) {
        const groupedExecutions = {};
        executions.forEach(execution => {
    ...
    calculateAverageExecutionTime(executions) {
        if (executions.length === 0)
            return 0;
        const completedExecutions = executions.filter(execution => execution.status === 'completed' && execution.startedAt && execution.completedAt);
    ...
    calculateAverageAgentExecutionTime(executions) {
        if (executions.length === 0)
            return 0;
        const completedExecutions = executions.filter(execution => execution.status === 'completed' && execution.startedAt && execution.completedAt);
        if (completedExecutions.length === 0)
    ...
    calculateAverageWorkflowExecutionTime(executions) {
        if (executions.length === 0)
            return 0;
        const completedExecutions = executions.filter(execution => execution.status === 'completed' && execution.startedAt && execution.completedAt);
        if (completedExecutions.length === 0)
    ...
    groupErrorsByDay(agentExecutions, workflowExecutions) {
        const groupedErrors = {};
    ...
    groupErrorsByType(agentExecutions, workflowExecutions) {
        const errorTypes = {
