...
class WorkflowService {
    constructor(io) {
        this.io = io;
        this.agentService = new agent_service_1.AgentService(io);
    ...
    async createWorkflow(userId, workflowData) {
        try {
            ...
            throw new errorHandler_1.ApiError(500, 'Failed to create workflow');
        }
    }
    ...
    async getWorkflows(userId) {
        try {
            ...
            throw new errorHandler_1.ApiError(500, 'Failed to get workflows');
        }
    }
    // Get workflow by ID
    async getWorkflowById(id, userId) {
        try {
            ...
            if (!workflow) {
                throw new errorHandler_1.ApiError(404, 'Workflow not found');
            }
            return workflow;
        }
        catch (error) {
            ...
            throw new errorHandler_1.ApiError(500, 'Failed to get workflow');
        }
    }
    // Update workflow
    async updateWorkflow(id, userId, workflowData) {
        try {
            ...
            if (!existingWorkflow) {
                throw new errorHandler_1.ApiError(404, 'Workflow not found or you do not have permission to update it');
            }
            ...
            throw new errorHandler_1.ApiError(500, 'Failed to update workflow');
        }
    }
    // Delete workflow
    async deleteWorkflow(id, userId) {
        try {
            ...
            if (!existingWorkflow) {
                throw new errorHandler_1.ApiError(404, 'Workflow not found or you do not have permission to delete it');
            }
            ...
            throw new errorHandler_1.ApiError(500, 'Failed to delete workflow');
        }
    }
    // Add agent to workflow
    async addAgentToWorkflow(workflowId, agentId, userId, position, config) {
        try {
            // Check if workflow exists and belongs to user
            const workflow = await prisma.workflow.findFirst({
                where: {
            ...
            throw new errorHandler_1.ApiError(500, 'Failed to add agent to workflow');
        }
    }
    // Remove agent from workflow
    async removeAgentFromWorkflow(workflowId, agentId, userId) {
        try {
            // Check if workflow exists and belongs to user
            const workflow = await prisma.workflow.findFirst({
                where: {
            ...
            if (!workflowAgent) {
                throw new errorHandler_1.ApiError(404, 'Agent not found in workflow');
            }
            // Remove agent from workflow
            ...
            throw new errorHandler_1.ApiError(500, 'Failed to remove agent from workflow');
        }
    }
    // Run workflow
    async runWorkflow(id, userId, input) {
        try {
            ...
            throw new errorHandler_1.ApiError(500, 'Failed to run workflow');
        }
    }
    // Execute workflow (internal method)
    async executeWorkflow(workflow, execution, input) {
        try {
            ...
            for (let i = 0; i < workflowAgents.length; i++) {
                ...
                (0, socket_1.emitWorkflowUpdate)(this.io, roomId, workflow.id, 'running', currentProgress);
                // Prepare agent input
                const agentInput = {
                    ...input,
                ...
                try {
                    ...
                    // Emit error event
                    (0, socket_1.emitWorkflowError)(this.io, roomId, workflow.id, `Error in agent ${agent.name}: ${agentError.message || 'Unknown error'}`);
                    return;
                }
            }
            ...
            (0, socket_1.emitWorkflowComplete)(this.io, roomId, workflow.id, result);
            ...
            // Emit error event
            const roomId = `workflow:${workflow.id}:execution:${execution.id}`;
            (0, socket_1.emitWorkflowError)(this.io, roomId, workflow.id, error instanceof Error ? error.message : 'Unknown error');
    ...
    // Run agent and wait for completion
    async runAgentAndWaitForCompletion(agentId, userId, input, executionId) {
    ...
    async getWorkflowExecutionStatus(executionId, userId) {
        try {
            ...
            if (!execution) {
                throw new errorHandler_1.ApiError(404, 'Execution not found');
            }
            // Check if user has access to the workflow
            if (execution.workflow.userId !== userId && !execution.workflow.isPublic) {
                throw new errorHandler_1.ApiError(403, 'You do not have permission to access this execution');
            }
            return execution;
        }
        catch (error) {
            ...
            throw new errorHandler_1.ApiError(500, 'Failed to get execution status');
        }
    }
    // Stop workflow execution
    async stopWorkflowExecution(executionId, userId) {
        try {
            const execution = await prisma.workflowExecution.findUnique({
                where: { id: executionId },
                include: {
            ...
            if (execution.workflow.userId !== userId) {
                throw new errorHandler_1.ApiError(403, 'You do not have permission to stop this execution');
            }
            ...
            if (execution.status === 'completed' || execution.status === 'failed' || execution.status === 'stopped') {
                throw new errorHandler_1.ApiError(400, `Execution is already ${execution.status}`);
            }
            ...
            (0, socket_1.emitWorkflowError)(this.io, roomId, execution.workflow.id, 'Execution stopped by user');
            ...
            throw new errorHandler_1.ApiError(500, 'Failed to stop execution');
        }
    }
    // Clone workflow
    async cloneWorkflow(id, userId, newName) {
        try {
            ...
            throw new errorHandler_1.ApiError(500, 'Failed to clone workflow');
        }
    }
    // Export workflow configuration
    async exportWorkflow(id, userId) {
        try {
            ...
            throw new errorHandler_1.ApiError(500, 'Failed to export workflow');
        }
    }
    // Import workflow configuration
    async importWorkflow(userId, importData) {
        try {
            // Validate import data
            if (!importData.name || !importData.config) {
                throw new errorHandler_1.ApiError(400, 'Invalid import data');
            }
            // Create the workflow
            const workflow = await prisma.workflow.create({
                data: {
            ...
            throw new errorHandler_1.ApiError(500, 'Failed to import workflow');
        }
    }
}
